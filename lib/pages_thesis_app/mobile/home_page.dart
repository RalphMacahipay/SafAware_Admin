...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  void checkIfSafe() {
    if (HomePage.alarmState!) {
      setState(() {
        ...
        //     Duration(seconds: int.parse(dropDownAlarm!));
      ...
      devstool.log("The HomePage().alarmState! is ${HomePage.alarmState}");
    } else {
      devstool.log("The HomePage().alarmState! is ${HomePage.alarmState}");
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    //-----------------------------------ContactPerson button-----------------
    const ringtoneBTN = PopupMenuItem<MenuAction>(
      value: MenuAction.ringtone,
      child: Text('Ringtone'),
    );
    //-----------------------------------About button-----------------
...
// TODO DAPAT DISABLE PAG WALA PA NA SET YUNG CURRENT LOCATION
      ...
      child: Container(
        ...
        child: Align(
          ...
          child: DropdownButton(
            ...
            items: <String>['Tricycle', '5', 'Multicab', 'Private Vehicle']
    ...
    //------------------------------Start BTN ---------------------
    // TODO DAPAT HINDI PA MAG SHOW PAG WALA PA NA SET YUNG LOCATION AND DESTINATION
    // TODO OFF DATAPAT YUNG FIND LOCATION, TYPES OF VEHICLE
            ...
            child: FloatingActionButton.extended(
              ...
              onPressed: isStartBtnOn!
                  ? () async {
                      ...
                      //TODO PAG OKAY NA YUNG PROFILE PARA MA NULL SAFETY
                      // Navigator.pushNamed(context, addCOntactPersonRoute);
                    }
                  : null,
            ),
      ...
      child: FloatingActionButton.extended(
        ...
        onPressed: isArriveBtnOn!
            ? () {
                ...
                devstool.log("isArriveBtnOn is $isArriveBtnOn");
                ...
                devstool.log("_locationSubscription is $_locationSubscription");
                devstool.log("isBTNTouch is $isBTNTouch");
              }
            : null,
      ),
    );
...
//  FIND LOCATION ------------------------------
  ...
  Future<void> _listenLocation() async {
    ...
    _locationSubscription = location.onLocationChanged.handleError((onError) {
      devstool.log("Error Occured");
      devstool.log(onError.toString());
      _locationSubscription?.cancel();
      setState(() {
        _locationSubscription = null;
      });
  ...
  _stopListening() {
    ...
    devstool.log("I'm Stop Listening");
  }

  _requestPermission() async {
    var status = await Permission.location.request();
    if (status.isGranted) {
      devstool.log("status Grandted");
      setState(() {
        isPermissionTrue = true;
        isBTNTouch = false;
      });
    } else if (status.isPermanentlyDenied) {
      devstool.log("status isPermanentlyDenied");
  ...
  _startTravel(AsyncSnapshot<QuerySnapshot> snapshot) {
    final Name = snapshot.data!.docs.singleWhere((element) =>
        element.id == _user.currentUser!.uid)["contactPerson.Name"];
    if (Name != null) {
      devstool.log("Has here Data");
      setPolyline();
      //TODO ILIPAT DOON SA PAG NAG SEARCH NG DESTINATION EH DRAW POLYLINE AGAD
      setState(() {
        isArriveBtnOn = true;
        isStartBtnOn = false;
        isOutPutCreated = true;
      ...
      devstool.log("isStartBtnOn is $isStartBtnOn");
